...
public class Ball : MonoBehaviour
{
    ...
    private Range<float> yVelocityRange = new Range<float>(-12, 12);
	...
	// current winner of previous round 
	...
	private void Serve ()
	{
		...
		if (winner == 0) 
		{
			GetComponent<Rigidbody>().velocity = new Vector3(Random.Range(xVelocityRange.Minimum, xVelocityRange.Maximum), Random.Range(2, yVelocityRange.Maximum), 0);
			Debug.Log("Blue Player Won");
		}
		// else if red player won
		else
		{
			GetComponent<Rigidbody>().velocity = new Vector3(Random.Range(xVelocityRange.Minimum, xVelocityRange.Maximum), Random.Range(yVelocityRange.Minimum, -2), 0);
		}
